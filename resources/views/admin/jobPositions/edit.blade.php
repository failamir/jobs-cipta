    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.job-positions.update", [$jobPosition->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('job') ? 'is-invalid' : '' }}" name="job_id" id="job_id">
                    @foreach($jobs as $id => $entry)
                        <option value="{{ $id }}" {{ (old('job_id') ? old('job_id') : $jobPosition->job->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('job'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('name_position') ? 'is-invalid' : '' }}" type="text" name="name_position" id="name_position" value="{{ old('name_position', $jobPosition->name_position) }}">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('description_position') ? 'is-invalid' : '' }}" name="description_position" id="description_position">{!! old('description_position', $jobPosition->description_position) !!}</textarea>
