            ...
            <div class="card-body">
                <form method="POST" action="{{ route("profile.password.updateProfile") }}">
                    ...
                    <div class="form-group">
                        ...
                        @endif
                        <select class="form-control select2 {{ $errors->has('roles') ? 'is-invalid' : '' }}" name="roles[]" id="roles" multiple required disabled>
                            @foreach($roles as $id => $role)
                                <option value="{{ $id }}" {{ (in_array($id, old('roles', [])) || $user->roles->contains($id)) ? 'selected' : $user->roles->contains($id)) }}>{{ $role }}</option>
                            @endforeach
                        </select>
                        @if($errors->has('roles'))
                            <div class="invalid-feedback">
...
<script>
    ...
    init: function () {
@if(isset($user) && $user->resume_cv)
      var file = {!! json_encode($user->resume_cv) !!}
...
<script>
    ...
    init: function () {
@if(isset($user) && $user->visa)
      var file = {!! json_encode($user->visa) !!}
          this.options.addedfile.call(this, file)
      file.previewElement.classList.add('dz-complete')
...
<script>
    ...
    init: function () {
@if(isset($user) && $user->passport)
      var file = {!! json_encode($user->passport) !!}
          this.options.addedfile.call(this, file)
      file.previewElement.classList.add('dz-complete')
...
<script>
    ...
    init: function () {
@if(isset($user) && $user->bst_ccm)
      var file = {!! json_encode($user->bst_ccm) !!}
          this.options.addedfile.call(this, file)
      file.previewElement.classList.add('dz-complete')
